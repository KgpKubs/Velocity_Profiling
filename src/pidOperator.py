...
def CallbackPID(msg):
    ...
    botAngle = homeBotTheta[0]
    print("BotAngle ", homeBotTheta[0])
    vXBot = vY*cos(botAngle) - vX*sin(botAngle)
    vYBot = vY*sin(botAngle) + vX*cos(botAngle)
...
def main():
    ...
    rospy.init_node('pidOperator', anonymous=True)
    t = rospy.get_rostime()
